...
export function formatUserGQL(mm, user) {
  const req = `
    ...
    ${user.lastName ? `lastName: "${formatGQLString(user.lastName)}"` : ""}
    ${user.language ? `language: "${formatGQLString(user.language)}"` : 'language: "en"'}
    ${user.otherNames ? `otherNames: "${formatGQLString(user.otherNames)}"` : ""}
    ${user.phoneNumber ? `phoneNumber: "${formatGQLString(user.phoneNumber)}"` : ""}
    ${user.birthDate ? `birthDate: "${formatGQLString(user.birthDate)}"` : ""}
    ...
    ${user.healthFacility ? `healthFacilityId: ${decodeId(user.healthFacility.id)}` : ""}
    ${user.roles ? `roles: [${user.roles.map((u) => decodeId(u.id))}]` : ""}
...
export function createUser(mm, user, clientMutationLabel) {
  const mutation = formatMutation("createUser", formatUserGQL(mm, user), clientMutationLabel);
  const requestedDateTime = new Date();
  return graphql(mutation.payload, ["ADMIN_USER_MUTATION_REQ", "ADMIN_USER_CREATE_RESP", "ADMIN_USER_MUTATION_ERR"], {
...
export function updateUser(mm, user, clientMutationLabel) {
  const mutation = formatMutation("updateUser", formatUserGQL(mm, user), clientMutationLabel);
  const requestedDateTime = new Date();
  return graphql(mutation.payload, ["ADMIN_USER_MUTATION_REQ", "ADMIN_USER_UPDATE_RESP", "ADMIN_USER_MUTATION_ERR"], {
    clientMutationId: mutation.clientMutationId,
    clientMutationLabel,
    requestedDateTime,
    userId: user.id,
